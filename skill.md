...
| Field      | Value                                           |
| ---------- | ----------------------------------------------- |
| Identifier | `outlinedriven-odin-claude-plugin-memory-update`                                |
| Version    | 1.0.1                                     |
...
**GitHub:** [OutlineDriven/odin-claude-plugin](https://github.com/OutlineDriven/odin-claude-plugin) — ⭐ 24 | Forks: 
...
Scans an agent's session-history transcripts to detect save-worthy signals (corrections, preferences, decisions, references) and propose memory files that can be confirmed and written into the agent's memory directory. Key features: runs resolve-paths.sh to locate MEMORY_DIR and SESSION_HISTORY_GLOB, executes scripts/scan-session.sh to produce a JSON list of proposals (type, slug, evidence_turn_ids, draft_body, draft_index_entry), renders human-readable proposals that cite the exact transcript turn, and waits for user confirmation before creating or revising memory files. The skill enforces filename and frontmatter conventions, requires additional sections for feedback/project types, and aborts on non-zero exits. Use cases: invoked interactively with “remember that…”, periodic session scans to capture preferences/decisions/references, or generating index entries for MEMORY.md. Core advantages: automated, auditable, schema-compliant memory creation while preventing fabrication; it does not delete, merge, or sanitize existing memory files.
...
```bash
npx -y @lobehub/market-cli skills install outlinedriven-odin-claude-plugin-memory-update
```
...
npx -y @lobehub/market-cli skills install outlinedriven-odin-claude-plugin-memory-update --agent open-claw

# Claude Code (installs to ./.claude/skills/)
npx -y @lobehub/market-cli skills install outlinedriven-odin-claude-plugin-memory-update --agent claude-code

# Codex (installs to ./.agents/skills/)
npx -y @lobehub/market-cli skills install outlinedriven-odin-claude-plugin-memory-update --agent codex

# Cursor (installs to ./.cursor/skills/)
npx -y @lobehub/market-cli skills install outlinedriven-odin-claude-plugin-memory-update --agent cursor
```

Install a specific version:

```bash
npx -y @lobehub/market-cli skills install outlinedriven-odin-claude-plugin-memory-update --version 1.0.1
```
...
```bash
npx -y @lobehub/market-cli skills rate outlinedriven-odin-claude-plugin-memory-update --score <1-5>
```
...
```bash
npx -y @lobehub/market-cli skills comment outlinedriven-odin-claude-plugin-memory-update -c "Your feedback here" --rating <1-5>
```
...
```bash
npx -y @lobehub/market-cli skills uncomment outlinedriven-odin-claude-plugin-memory-update
```
...
```bash
npx -y @lobehub/market-cli skills comments outlinedriven-odin-claude-plugin-memory-update
```
